...
// running sunnyinit on legacy SAR AVD emulator
bool avd_hack = false;
...
void SunnyInit::collect_devices() const noexcept {
    char path[PATH_MAX];
    devinfo dev{};
...
uint64_t SunnyInit::find_block(const char *partname) const noexcept {
    if (dev_list.empty())
        collect_devices();
...
void SunnyInit::mount_preinit_dir() noexcept {
    ...
    // After the device boots up, sunnyd will properly symlink the correct path at PREINITMIRR as writable.
...
bool SunnyInit::mount_system_root() noexcept {
...
mount_root:
    ...
    // special hacks in sunnyinit to work properly.
...
void SunnyInit::setup_tmp(const char *path) noexcept {
    LOGD("Setup Sunny tmp at %s\n", path);
    chdir("/data");
    ...
    cp_afc(".backup/.sunny", MAIN_CONFIG);
    rm_rf(".backup");

    // Create applet symlinks
    for (int i = 0; applet_names[i]; ++i)
        xsymlink("./sunny", applet_names[i]);
    xsymlink("./sunnypolicy", "supolicy");
    ...
    // Prepare worker
    xmount("sunny", WORKERDIR, "tmpfs", 0, "mode=755");
