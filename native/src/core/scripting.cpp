...
static void set_script_env() {
    ...
    char new_path[4096];
    ssprintf(new_path, sizeof(new_path), "%s:%s", getenv("PATH"), get_sunny_tmp());
    setenv("PATH", new_path, 1);
    if (SunnyD::Get().zygisk_enabled())
...
APK=%s
log -t Sunny "pm_install: $APK"
log -t Sunny "pm_install: $(pm install -g -r $APK 2>&1)"
appops set %s REQUEST_INSTALL_PACKAGES allow
rm -f $APK
)EOF";

void install_apk(rust::Utf8CStr apk) {
    setfilecon(apk.c_str(), SUNNY_FILE_CON);
...
PKG=%s
log -t Sunny "pm_uninstall: $PKG"
log -t Sunny "pm_uninstall: $(pm uninstall $PKG 2>&1)"
)EOF";
...
PKG=%s
USER=%d
log -t Sunny "pm_clear: $PKG (user=$USER)"
log -t Sunny "pm_clear: $(pm clear --user $USER $PKG 2>&1)"
)EOF";
...
void install_module(const char *file) {
    ...
    if (access(DATABIN, F_OK) ||
        ...
        abort(stderr, "Incomplete Sunny install");
    if (access(file, F_OK))
    ...
    setenv("ASH_STANDALONE", "1", 1);
    setenv("SUNNYTMP", get_sunny_tmp(), 0);
    free(zip);
