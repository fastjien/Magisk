use crate::consts::{DATABIN, LOG_PIPE, SUNNY_LOG_CON, SUNNYDB, MODULEROOT, SECURE_DIR};
...
pub(crate) fn restorecon() {
    ...
    unsafe { libc::chmod(cstr!(SUNNYDB).as_ptr(), 0o000) };
}

pub(crate) fn restore_tmpcon() -> LoggedResult<()> {
    let tmp = get_sunny_tmp();
    if tmp == "/sbin" {
    ...
    path.set_secontext(cstr!(SUNNY_LOG_CON))?;
