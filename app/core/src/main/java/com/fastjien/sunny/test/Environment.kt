...
import com.fastjien.sunny.core.BuildConfig.APP_PACKAGE_NAME
import com.fastjien.sunny.core.Const
import com.fastjien.sunny.core.download.DownloadNotifier
import com.fastjien.sunny.core.download.DownloadProcessor
import com.fastjien.sunny.core.ktx.cachedFile
import com.fastjien.sunny.core.model.module.LocalModule
import com.fastjien.sunny.core.tasks.AppMigration
...
class Environment : BaseTest {
    ...
    companion object {
        ...
        fun preinit(): Boolean {
            return Shell.cmd("sunny --preinit-device").exec().isSuccess
        }
    ...
    private fun checkModuleZip(file: File) {
        ...
        ZipFile.Builder().setFile(file).get().use { zip ->
            ...
            assertArrayEquals(MODULE_ERROR, "#SUNNY\n".toByteArray(), script)
        }
    }
    ...
    private fun setupSepolicyRuleModule(root: ExtendedFile) {
        ...
        PrintStream(path.getChildFile("sepolicy.rule").newOutputStream()).use {
            it.println("type sunny_test domain")
        }
    ...
    fun setupEnvironment() {
        runBlocking {
            assertTrue(
                "Sunny setup failed",
                SunnyInstaller.Emulator(TimberLog, TimberLog).exec()
            )
        }
