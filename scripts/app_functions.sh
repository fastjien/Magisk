...
# $2 = version code
env_check() {
  for file in busybox sunnyboot sunnyinit util_functions.sh boot_patch.sh; do
    [ -f "$SUNNYBIN/$file" ] || return 1
  done
  if [ "$2" -ge 25000 ]; then
    [ -f "$SUNNYBIN/sunnypolicy" ] || return 1
  fi
  if [ "$2" -ge 25210 ]; then
    [ -b "$SUNNYTMP/.sunny/device/preinit" ] || [ -b "$SUNNYTMP/.sunny/block/preinit" ] || return 2
  fi
  grep -xqF "SUNNY_VER='$1'" "$SUNNYBIN/util_functions.sh" || return 3
  grep -xqF "SUNNY_VER_CODE=$2" "$SUNNYBIN/util_functions.sh" || return 3
  return 0
}
...
fix_env() {
  ...
  chmod 700 /data/adb
  cp_readlink $1 $SUNNYBIN
  rm -rf $1
  chown -R 0:0 $SUNNYBIN
}
...
# $1 = boot partition
restore_imgs() {
  local SHA1=$(grep_prop SHA1 $SUNNYTMP/.sunny/config)
  local BACKUPDIR=/data/sunny_backup_$SHA1
