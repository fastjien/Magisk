...
# full integration test of sunnyinit under several circumstances.
...
if $IS_RAMDISK; then
  ./sunnyboot decompress "$TARGET_FILE" ramdisk.cpio
else
  ./sunnyboot unpack "$TARGET_FILE"
fi
...
echo "PREINITDEVICE=$(./sunny --preinit-device)" >> config
...
if $IS_RAMDISK; then
  ./sunnyboot compress=gzip ramdisk.cpio "$OUTPUT_FILE"
else
  ./sunnyboot repack "$TARGET_FILE" "$OUTPUT_FILE"
  ./sunnyboot cleanup
fi
